...
public class CheckIdenticalBSTs {
	private boolean checkIfSameBSTs(int[] arrayForTree1, int[] arrayForTree2, int index1, int index2, int min,
			int max) {
		/*
		 * find the first element between min and max. that element would be
		 * used as the root of the subtree we are looking to construct
		 */
		int i, j;
		 ...
		 * element found should be same. If element not found, then that means
		 * we found no node satisfying this condition. If this is the case then
		 * this should be the case for both the trees
		 */
		...
		// if current nodes are same then check if left and right sub-trees for
